...
namespace Light.Managed.Tools
{
    ...
    {
        const char LeadByteChar = '\uFFFE';
        char[] _dbcsToUnicode;
        ushort[] _unicodeToDbcs;
        string _webName;

        static readonly Dictionary<string, Tuple<char[], ushort[]>> Cache;
        ...
        int GetCharCount(byte[] bytes, int index, int count, DbcsDecoder decoder)
        {
            var charCount = 0;
        ...
        int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex, DbcsDecoder decoder)
        {
            var charCount = 0;
        ...
        sealed class DbcsDecoder : Decoder
        {
            readonly DbcsEncoding _encoding;
