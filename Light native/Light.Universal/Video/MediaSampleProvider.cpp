...
HRESULT MediaSampleProvider::WriteAVPacketToStream(DataWriter^ dataWriter, AVPacket* avPacket)
{
	...
	auto aBuffer = CreateInternalBuffer(avPacket->data, avPacket->size);//ref new Platform::Array<uint8_t>(avPacket->data, avPacket->size);
	dataWriter->WriteBuffer(aBuffer);
	return S_OK;
}
